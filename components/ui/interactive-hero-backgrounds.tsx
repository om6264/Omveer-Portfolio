...
import {
  Clock, PerspectiveCamera, Scene, WebGLRenderer, SRGBColorSpace,
  MathUtils, Vector2, Vector3, MeshPhysicalMaterial, Color, Object3D,
...
class SceneManager {
  ...
  #animationState: { elapsed: number; delta: number } = { elapsed: 0, delta: 0 };
...
class Spheres extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: any) {
    ...
    const envTexture = pmrem.fromScene(new (RoomEnvironment as any)(renderer)).texture;
    pmrem.dispose();
  ...
  setColors(colors: (string | Color)[]): void {
...
export const InteractiveHero = ({
  ...
  useEffect(() => {
    ...
    const three = new SceneManager({ canvas, size: "parent" });
